...
from optunaz.algorithms import chem_prop, calibrated_cv
...
def test_ChemPropRegressor():
    model, train_score, test_score = getmodel(
        "examples/building/ChemProp_regression_build.json", "molwt"
    )
    assert isinstance(model, chem_prop.ChemPropRegressor)
...
def test_Calibration_ChemProp(clean_shared_datadir):
    model, train_score, test_score = getmodel(
        "examples/building/calibration_chemprop_build.json", "molwt_gt_330"
    )
    assert isinstance(model, calibrated_cv.CalibratedClassifierCVWithVA)
    with open(str(clean_shared_datadir / "test.pkl"), "wb") as fid:
        dill.dump(model, fid)
