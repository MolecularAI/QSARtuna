...
def test_physchem_descriptor(clean_shared_datadir):
    file_drd2_300 = str(clean_shared_datadir / "DRD2" / "subset-50" / "train.csv")
    config = OptimizationConfig(
        ...
        algorithms=[
            ...
            XGBRegressor.new(
                n_estimators=XGBRegressor.Parameters.XGBRegressorNEstimators(low=2, high=2)
            ),
            PLSRegression.new(),
        ],
...
def test_physchem_scaler(clean_shared_datadir):
    p = clean_shared_datadir / "DRD2/subset-50/train.csv"
...
def test_physchem_noscaler(clean_shared_datadir):
    p = clean_shared_datadir / "DRD2/subset-50/train.csv"
...
def test_serialization(clean_shared_datadir):
    p = clean_shared_datadir / "DRD2/subset-50/train.csv"
    d = ScaledDescriptor.new(
        descriptor=UnscaledPhyschemDescriptors.new(),
        scaler=UnfittedSklearnScaler(
...
def test_scaledphyschem(clean_shared_datadir):
    p = clean_shared_datadir / "DRD2/subset-50/train.csv"
    d = PhyschemDescriptors.new(
        scaler=UnfittedSklearnScaler(
            mol_data=UnfittedSklearnScaler.MolData(
