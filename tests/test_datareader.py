...
from optunaz.utils.preprocessing.deduplicator import KeepAllNoDeduplication, KeepFirst
...
@pytest.fixture
def drd2_aux(clean_shared_datadir):
    return str(clean_shared_datadir / "aux_descriptors_datasets" / "train_with_conc.csv")
...
@pytest.fixture
def peptide_toxinpred3(clean_shared_datadir):
    return str(clean_shared_datadir / "peptide" / "toxinpred3" / "train.csv")
...
def test_split_and_extra(drd2_300, clean_shared_datadir):
    testfile = str(clean_shared_datadir / "DRD2" / "subset-50" / "test.csv")
    data = Dataset(
        input_column="canonical",
        response_column="molwt",
        training_dataset_file=drd2_300,
...
@pytest.fixture
def drd2_reg_errs(clean_shared_datadir):
    return str(
        clean_shared_datadir / "DRD2" / "subset-50" / "train_response_col_issue_test.csv"
    )
...
@pytest.fixture
def drd2_cls_errs(clean_shared_datadir):
    return str(
        clean_shared_datadir / "DRD2" / "subset-50" / "train_response_col_issue_test.csv"
    )
...
def test_cls_errs_modi(drd2_cls_errs):
    data = Dataset(
        input_column="canonical",
        response_column="molwt_gt_330",
        response_type="classification",
...
@pytest.mark.parametrize(
    "covariate_column,exp_tr,exp_te",
    [
        ("aux1", 40, 10),
        ("aux2", 80, 20),
        ("aux3", 80, 20),
...
def test_aux(drd2_aux, covariate_column, exp_tr, exp_te):
    data = Dataset(
        input_column="canonical",
        response_column="molwt_gt_330",
        response_type="classification",
...
def test_peptide_modi(peptide_toxinpred3):
    data = Dataset(
        input_column="Smiles",
        response_column="Class",
        training_dataset_file=peptide_toxinpred3,
