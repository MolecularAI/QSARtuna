...
@pytest.fixture
def optconfig_classification(file_drd2_50, file_drd2_50_side_info_cls):
    return OptimizationConfig(
        data=Dataset(
            input_column="canonical",
...
@pytest.fixture
def optconfig_classification_X_aux(file_drd2_50, file_drd2_50_side_info_cls):
    return OptimizationConfig(
        data=Dataset(
            input_column="canonical",
            response_column="molwt_gt_330",
...
def test_chemprop_with_XGB(file_drd2_50):
    config = OptimizationConfig(
        ...
        descriptors=[ECFP.new(), SmilesFromFile.new()],
        algorithms=[ChemPropRegressor.new(epochs=1), XGBRegressor.new()],
        settings=OptimizationConfig.Settings(
            mode=ModelMode.REGRESSION,
            n_splits=5,
            n_trials=3,
