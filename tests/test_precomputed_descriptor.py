...
def test_from_existing_file(clean_shared_datadir):
    file = clean_shared_datadir / "precomputed_descriptor" / "tiny.csv"
    d = PrecomputedDescriptorFromFile.new(
...
def test_3(clean_shared_datadir):
    ori_molfile = clean_shared_datadir / "DRD2" / "subset-50" / "train.csv"
...
def test_4(clean_shared_datadir):
    ...
    config.data.training_dataset_file = f.name
    config.descriptors[0].parameters.file = f.name
    config.data.covariate_column = "aux"
...
def test_scaled_precomputed(clean_shared_datadir):
    testargs = [
        "prog",
        "--config",
        str(
            attach_root_path(
                "examples/optimization/drd2_50_precomputed_descriptor_scaled.json"
            )
        ),
    ...
    predict_args = [
        ...
        str(clean_shared_datadir / "precomputed_descriptor/train_with_fp.csv"),
