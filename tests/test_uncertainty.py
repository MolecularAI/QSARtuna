...
from optunaz.descriptors import ECFP, SmilesFromFile, UnscaledPhyschemDescriptors
from optunaz.predict import UncertaintyError
...
@pytest.fixture
def file_drd2_50_err(clean_shared_datadir):
    """Returns 50 molecules from DRD2 dataset."""
    return str(clean_shared_datadir / "DRD2" / "subset-50" / "train_with_errors.csv")


@pytest.fixture
def pretrained_cp(clean_shared_datadir):
    """Returns 50 molecules from DRD2 dataset."""
    return str(clean_shared_datadir / "DRD2" / "drd2_reg.pkl")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            XGBClassifier.new(
                n_estimators=XGBClassifier.Parameters.XGBClassifierNEstimators(
                    low=2, high=2
                )
            ),
...
def test_CalibratedClassifierCVWithVA_uncertainty(
    ...
    predictions = pd.read_csv(
        str(clean_shared_datadir / "outprediction"), usecols=["Prediction_uncert"]
    )
...
def optconfig_mapie_reg(file_drd2_50, estimator, descriptor):
    return OptimizationConfig(
        ...
        descriptors=[descriptor],
        algorithms=[
            MapieRegressor.new(estimator=estimator, n_folds=2, test_size=0.05),
        ],
        settings=OptimizationConfig.Settings(
...
def test_mapie_reg_uncertainty(
    file_drd2_50,
    clean_shared_datadir,
    estimator,
    descriptor,
):
    optconfig = optconfig_mapie_reg(file_drd2_50, estimator, descriptor)
    optconfig.set_cache()
...
def optconfig_mapie_cls(file_drd2_50, estimator, descriptor):
    return OptimizationConfig(
        ...
        descriptors=[descriptor],
        algorithms=[
            MapieClassifier.new(estimator=estimator, n_folds=4, test_size=0.15),
        ],
        settings=OptimizationConfig.Settings(
            mode=ModelMode.CLASSIFICATION,
            n_splits=2,
...
def test_mapie_cls_uncertainty(
    file_drd2_50,
    clean_shared_datadir,
    estimator,
    descriptor,
):
    optconfig = optconfig_mapie_cls(file_drd2_50, estimator, descriptor)
    optconfig.set_cache()
    ...
    assert len(predictions.dropna()) == 11
...
def test_pretrained_uncertainty(
    file_drd2_50, file_drd2_50_err, clean_shared_datadir, pretrained_cp
):
    predict_args = [
        "prog",
        "--model-file",
...
def optconfig_tabpfn(file_drd2_50, estimator, descriptor):
    return OptimizationConfig(
        data=Dataset(
            input_column="canonical",
            response_column="molwt",
...
def test_tabpfn_uncertainty(
    ...
    optconfig = optconfig_tabpfn(file_drd2_50, estimator, descriptor)
