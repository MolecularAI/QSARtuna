...
@pytest.fixture
def conf_conc(clean_shared_datadir):
    conf = load_json.loadJSON(
        path=os.path.join(
            files_paths.move_up_directory(__file__, 1),
            "examples",
...
@pytest.fixture
def confChemProp(clean_shared_datadir):
    conf = load_json.loadJSON(
        path=os.path.join(
            files_paths.move_up_directory(__file__, 1),
            "examples",
...
@pytest.fixture
def conf_peptide(clean_shared_datadir):
    conf = load_json.loadJSON(
        path=os.path.join(
            files_paths.move_up_directory(__file__, 1),
            "examples",
    ...
    conf["data"]["training_dataset_file"] = str(
        clean_shared_datadir / "peptide/toxinpred3/subset-50/train.csv"
    )
    conf["data"]["test_dataset_file"] = str(
        clean_shared_datadir / "peptide/toxinpred3/subset-50/test.csv"
    )
    return conf


@pytest.fixture
def conf_peptide_reg(clean_shared_datadir):
    conf = load_json.loadJSON(
        path=os.path.join(
            files_paths.move_up_directory(__file__, 1),
            "examples",
    ...
    conf["data"]["training_dataset_file"] = str(
        clean_shared_datadir / "peptide/permeability/train.csv"
    )
    conf["data"]["test_dataset_file"] = str(
        clean_shared_datadir / "peptide/permeability/train.csv"
    )
...
@pytest.fixture
def train_with_conc(clean_shared_datadir):
    return str(
        clean_shared_datadir / "aux_descriptors_datasets" / "train_with_conc.csv"
    )
...
@pytest.fixture
def train_with_si(clean_shared_datadir):
    return str(clean_shared_datadir / "DRD2" / "subset-50" / "train_side_info.csv")
...
def test_AvalonCount(conf):
    # overwrite descriptor specification
    confDict = conf
    confDict[_OC.DESCRIPTORS] = [
        {
            "name": _OC.DESCRIPTORS_AVALONCOUNT,
            _OC.GENERAL_PARAMETERS: {_OC.DESCRIPTORS_AVALONCOUNT_NBITS: 2048},
        }
    ]
...
def test_JazzyDescriptors2D(conf):
    # overwrite descriptor specification
    confDict = conf
    confDict[_OC.DESCRIPTORS] = [
        {
...
def test_JazzyDescriptors3D(conf):
    # overwrite descriptor specification
    confDict = conf
    confDict[_OC.DESCRIPTORS] = [
        {
            "name": _OC.DESCRIPTORS_JAZZY,
            _OC.GENERAL_PARAMETERS: {
                _OC.DESCRIPTORS_JAZZY_EMBEDDINGTYPE: "3D",
                _OC.DESCRIPTORS_JAZZY_EMBEDDINGSEED: 123,
                _OC.DESCRIPTORS_JAZZY_EMBEDDINGMAXITERATIONDS: 2,
            },
        }
    ]
...
def test_Mordred(conf):
    # overwrite descriptor specification
    confDict = conf
    confDict[_OC.DESCRIPTORS] = [
        {"name": _OC.DESCRIPTORS_MORDRED, _OC.GENERAL_PARAMETERS: {}}
    ]
...
def test_PrecomputedDescriptorFromFileWithAux(conf_conc, train_with_conc):
    ...
    conf_conc.data.covariate_column = "aux2"
    ...
    confDict[_OC.DESCRIPTORS] = [
        {
            ...
            _OC.GENERAL_PARAMETERS: {
                ...
                _OC.DESCRIPTORS_SMILES_AND_SI_Y_AUX_COLUMN: "ylabels",
                _OC.DESCRIPTORS_SMILES_AND_SI_X_AUX_COLUMN: "xlabels",
            },
        }
    ]
