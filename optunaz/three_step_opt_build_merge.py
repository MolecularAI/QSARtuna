...
def base_chemprop_params(alg):
    ...
    A Check is performed to ensure any parameters outside the predefined subspace are popped from fixed parameters.
    """
    ...
    for param in list(fixed_params.keys()):
        ...
        # Parameters with a low and high Optuna optimization window outside the user defined subspace are dropped here.
        # This ensures that the original authors suggestions for a given parameter are only used if they are within the
        # user defined subspace. By popping the parameter, Optuna will suggest a value within the user defined subspace
        # and not the original authors suggestion.
        ...
        # Recommended items not within enum of the user config are dropped here
            ...
            try:
                # Convert any bool values into int for enum compatibility (bool not allowed for enum)
                if type(fixed_params[param]) == bool:
                    fixed_params[param] = int(fixed_params[param])
    ...
    if n_trials >= 1:
        for alg in optconfig.algorithms:
            if isinstance(alg, Union[ChemPropClassifier, ChemPropRegressor]):
                # Initial ChemProp trials directed to values specified by original authors, where the user config allows
                sensible_default = base_chemprop_params(alg)
                study.enqueue_trial(sensible_default)
                logging.info(
                    f"Enqueued ChemProp manual trial with these sensible defaults these parameters: {sensible_default}"
                )
    ...
    if optconfig.settings.split_chemprop:
        ...
        try:
            ...
            for cfg_idx, cfg in enumerate(split_optimize(optconfig)):
                ...
                # set parameters for next chemprop study (currently share n_startup_trials with n_trials)
                if cfg_idx == 0:
                    studies = study
                    trial_number_offset = len(study.get_trials())
                    n_startup_trials = math.floor(n_chemprop_trials / 2)
