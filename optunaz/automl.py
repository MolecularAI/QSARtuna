...
class ModelDispatcher:
    """
    Use ModelAutoML config as a basis to prepare Qptuna jobs, dispatching to SLURM.
        ...
        if not self._pretrained_model:
            try:
                with open(self.prev_model_name, "rb") as newf:
                    self._pretrained_model = dill.load(newf)
    ...
    def checkisLocked(self, taskcode):
        ...
        Locks occur if Qptuna is unable to run multiple retrain script instances run.
        """
    ...
    def doTemporalPredictions(self, new_data):
        """
        Start/check temporal predictions with an old Qptuna model vs. newest data
        """
    ...
    def writeSlurm(self):
        """
        Writes a slurm job for a Qptuna run for a given taskcode
        """
    ...
    def writeJson(self):
        """
        Writes a Qptuna json for a given taskcode
        """
    ...
    def checkSlurmStatusAndNextProcedure(self):
        ...
        try:
            ...
            if "DUE TO TIME LIMIT" in slurm_log:
                ...
                logging.debug(
                    f"{self.slurm_name} does not support uncertainty estimation"
                )
                raise SlurmJobSkip
        except FileNotFoundError:
            raise SlurmNoLog
...
def meta():
    """
    Tracks temporal performance of Qptuna models by writing the metadata to JSON files
    """
    ...
    requiredNamed.add_argument("--pkl", type=str, help="Path to the output Qptuna PKL file", required=True)
...
def main():
    start = time.time()
    parser = argparse.ArgumentParser(
        description="AutoML scheduling for temporal automatic retraining of models"
    )
    ...
    requiredNamed.add_argument("--user_name", type=str, help="PRID for the AutoML user", required=True)
    ...
    parser.add_argument('--slurm-req-mem', type=int, default=None) # By default None means dynamic mem resource allocation
