...
def convert(
    ...
    if isinstance(loaded, QptunaModel):
        if json_descriptor is not None:
            logging.warning(
                "Since input model is from Qptuna, json_descriptor was supplied but will be ignored"
            )
    ...
    if wrap_for_uncertainty:
        algorithm = convertedmodel.new(model_file=str(pkl_estimator), refit_model=1)
        buildinfo = prep_build(mode, pkl_estimator, algorithm, descriptor)
        if mode == ModelMode.REGRESSION:
            from optunaz.config.buildconfig import MapieRegressor
            ...
            algorithm = CalibratedClassifierCVWithVA.new(
                estimator=algorithm, n_folds=3, ensemble="True", method="vennabers"
            )
        algorithm = algorithm.estimator()
        algorithm.train_smiles_ = loaded.predictor.train_smiles_
        algorithm.X_ = loaded.predictor.X_
        algorithm.y_ = loaded.predictor.y_
        algorithm.aux_ = loaded.predictor.aux_
        algorithm.test_smiles_ = loaded.predictor.test_smiles_
        algorithm.test_X_ = loaded.predictor.test_X_
        algorithm.test_y_ = loaded.predictor.test_y_
        algorithm.test_aux_ = loaded.predictor.test_aux_
        algorithm.fit(loaded.predictor.X_, loaded.predictor.y_)
        model = save_model(algorithm, buildinfo, out_name, None, None)
    else:
        algorithm = convertedmodel.new(model_file=str(pkl_estimator), refit_model=0)
        buildinfo = prep_build(mode, pkl_estimator, algorithm, descriptor)
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert an existing sklearn(-like) model into a Qptuna model"
    )
