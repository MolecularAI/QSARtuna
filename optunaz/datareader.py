...
from apischema.metadata import none_as_undefined, skip
...
from optunaz.utils.preprocessing.deduplicator import Deduplicator, KeepMedian
...
class Dataset:
    ...
    covariate_column: Optional[str] = field(
        default=None,
        metadata=schema(
            title="Covariate descriptor column",
            description="Name of covariate column with additional values that the model use to predict (not to be confused with auxiliary descriptors).",
        )
        | none_as_undefined,
    )
    ...
    assess_modelability: bool = field(
        default=False,
        metadata=schema(
            title="Assess modelability",
            description="Assess the modelability of the dataset using the MODI, a set of metrics based on the work of "
            "Golbraikh et al. DOI: 10.1021/ci400572x, to produce a set of metrics which assess the "
            "modelability of a dataset.  The metrics are calculated using a leave-one-out "
            "cross-validation approach, after the data is preprocessed but before optimization / modelling",
        ),
    )
    ...
    def _initialize_sets(self):
        ...
        if self.covariate_column and self.aux_transform is not None:
        ...
        if self.save_intermediate_files:
            ...
            if train_auxs is not None:
                str_tr_auxs = [
                    np.array2string(row, separator=",").strip("[]")
                    if hasattr(train_auxs, "shape") and len(train_auxs.shape) > 1
                    else row
                    for row in train_auxs
                ]
            else:
                str_tr_auxs = train_auxs

            if test_auxs is not None:
                str_te_auxs = [
                    np.array2string(row, separator=",").strip("[]")
                    if hasattr(test_auxs, "shape") and len(test_auxs.shape) > 1
                    else row
                    for row in test_auxs
                ]
            else:
                str_te_auxs = test_auxs

            pd.DataFrame(
                data={
                    "train_smiles": train_smiles,
                    "train_y": train_y,
                    "train_aux": str_tr_auxs,
                }
        ...
        if self.assess_modelability and self.response_type:
            from optunaz.utils.preprocessing.modi import assess_modelability

            logger.info(f"Performing MODI modelability analysis, please wait")
            modi = assess_modelability(
                train_smiles,
                train_y,
                train_auxs,
                test_smiles,
            ...
            logger.info(f"MODI modelability analysis results: {modi}")
            self._modi = modi
        else:
            self._modi = None
            logger.info(f"MODI not performed")
