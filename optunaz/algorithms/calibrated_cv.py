...
from sklearn.model_selection import StratifiedKFold, check_cv, cross_val_predict
from sklearn.utils import Bunch, _safe_indexing, column_or_1d, indexable
...
class CalibratedClassifierCVWithVA(CalibratedClassifierCV):
    """Custom implementation of `CalibratedClassifierCV` that integrates VennABERS calibration.

    This class extends the functionality of Scikit-learn's `CalibratedClassifierCV` to support
    VennABERS calibration, enabling uncertainty estimation for classification tasks. It provides
    additional methods and parameters for handling VennABERS calibration alongside isotonic and
    sigmoid methods.

    The following attributes are expected:
        estimator (estimator implementing `fit` and `predict_proba` or `decision_function`).
        method (calibration method to use. Options are "isotonic", "sigmoid", or "vennabers").
        n_folds (no. folds for the cross-validation used to get calibration sets).
        n_jobs (no. jobs to run in parallel)
        ensemble (whether to use ensemble calibration - Using ensembles fits a copy of the base estimator
            to the training subset for each split, and calibrates it using the testing subset. Predicted
            probabilities are averaged across the calibrated classifiers.

            If ensembles are not used then cross-validation is used to obtain unbiased predictions,
            via cross_val_predict, which are then used for calibration. In this case the base estimator,
            trained on all available data is used for inference.
        seed (seed for reproducibility)
        cv (cross-validation strategy to use in calibration splits.

    The _predict_uncert method provides uncertainty estimates based on the VennABERS interval.
    NB: Only availablewhen the calibration method is set to "vennabers".
    """
    ...
    def __init__(
        ...
        *,
        ...
        if self.method == "vennabers":
            self.predict_uncert = self._predict_uncert
...
class _VennAbersCalibration(RegressorMixin, BaseEstimator):
    """
...
def _fit_calibrator_with_va(clf, predictions, y, classes, method, sample_weight=None):
    """
    Modification to fit_calibrator Scikit-learn function to add VennABERS via adding the
    _VennAbersCalibration to the method list
    """
...
def _fit_classifier_calibrator_pair_with_va(
    ...
    via the _fit_calibrator_with_va method
    """
