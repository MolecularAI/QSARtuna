...
def build_with_al(model_path, inference_path, mode):
    ...
    predict_args = [
        ...
        "--input-smiles-csv-file", str(inference_path),
        "--input-smiles-csv-column", "Structure",
        "--input-precomputed-file", str(inference_path),
        "--input-precomputed-input-column", "Structure",
        "--input-precomputed-response-column", "AZlogD74",
        "--output-prediction-csv-file", f"{os.path.dirname(model_path)}/al.csv",
        "--predict-uncertainty",
    ...
    try:
        ...
        logger.info(
            f"File not found during prediction on stock, AL predictions not performed: {e}"
        )
        ...
        logging.info(
            "Prediction on stock not performed: algorithm requires corvariate auxiliary data for inference"
        )
...
def main():
    ...
    parser.add_argument(
        "--inference_uncert",
        help="Apply active learning at build time for a file path of SMILES. AL selection is bsed on uncertainty inference.",
        type=pathlib.Path,
        default=None,
    )
    ...
    if isinstance(config, OptimizationConfig):
        ...
        build_al = False
        cache = config._cache if not args.no_cache else None
        cache_dir = config._cache_dir if not args.no_cache else None
    ...
    if args.best_model_outpath:
        ...
        if not args.merged_model_outpath and build_al and args.inference_uncert:
            build_with_al(
                args.best_model_outpath,
    ...
    if args.merged_model_outpath:
        ...
        if build_al and args.inference_uncert:
            build_with_al(
                args.merged_model_outpath,
                args.inference_uncert,
                buildconfig.settings.mode,
